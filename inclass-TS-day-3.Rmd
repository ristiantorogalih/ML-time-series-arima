...
Trend: cenderung linear -> additive
Seasonality: varians konstan -> additive
...
Kesimpulan: Data `no_train` perlu dilakukan differencing sebanyak ... hingga stasioner.
...
- auto: ARIMA(0,1,1) -> MAPE ...
- manual: ARIMA(1,1,2) -> MAPE ...
...
- Training MAPE = ...% sedangkan Testing MAPE = ...%
...
- ARIMA Workflow:
  ...
  3. Uji stasioner: dengan ..., harapannya p-value ... alpha agar data stasioner
  4. Apabila belum stasioner, maka lakukan ... hingga stasioner
  5. Buat baseline model dengan `auto.arima(obj_ts, seasonal=F)`
  6. Tuning model dengan menentukan order p dan q secara manual
    + Order p ditentukan dari plot ...
    + Order q ditentukan dari plot ...
    + Memperhatikan pola ... dan ... dari plot tersebut
...
sales_test <- tail(sales_ts, ...)

# 7 tahun untuk train, sisanya
sales_train <- head(sales_ts, ...)
```
...
```{r}
sales_auto_f <- forecast(sales_auto, h = ...)
sales_sarima1_f <- 
```
